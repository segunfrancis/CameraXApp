...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val luminousValue: TextView = findViewById(R.id.luminous_value)
    ...
    private fun startCamera() {
        ...
        // Setup image analyser pipeline that computes average pixel luminance
        val analyzerConfig = ImageAnalysisConfig.Builder().apply {
            // Use a worker thread for image analysis to prevent glitches
            val analyzerThread = HandlerThread("LuminosityAnalysis").apply { start() }
            setCallbackHandler(Handler(analyzerThread.looper))
            // In our analysis, we care more about the latest image than analysing every image
            setImageReaderMode(ImageAnalysis.ImageReaderMode.ACQUIRE_LATEST_IMAGE)
        }.build()

        // Build the image analysis use case and instantiate our analyser
    ...
    private class LuminosityAnalyzer : ImageAnalysis.Analyzer {
        ...
        */
         ...
         * the current target rotation of [ImageAnalysis], expressed as
